...
def test_position_reader_loads_only_active_positions():
    ...
    mapping = {
        (pm, ArbitrumUniswapV3PositionReader._BALANCE_OF(owner).lower()): _words_hex(_uint_word(2)),
        (pm, ArbitrumUniswapV3PositionReader._TOKEN_OF_OWNER_BY_INDEX(owner, 0).lower()): _words_hex(_uint_word(1)),
        (pm, ArbitrumUniswapV3PositionReader._TOKEN_OF_OWNER_BY_INDEX(owner, 1).lower()): _words_hex(_uint_word(2)),
        (pm, ArbitrumUniswapV3PositionReader._POSITIONS(1).lower()): p1,
        (pm, ArbitrumUniswapV3PositionReader._POSITIONS(2).lower()): p2,
        (factory, ArbitrumUniswapV3PositionReader._GET_POOL(token0, token1, 3000).lower()): _words_hex(_address_word(pool)),
    ...
    async def price_request_fn(tokens, _timeout):
        return {str(token).lower(): {"usd": 1.0 if str(token).lower() == token0.lower() else 3000.0} for token in tokens}
...
def test_position_reader_marks_state_stale_when_slot0_unavailable():
    ...
    mapping = {
        (pm, ArbitrumUniswapV3PositionReader._BALANCE_OF(owner).lower()): _words_hex(_uint_word(1)),
        (pm, ArbitrumUniswapV3PositionReader._TOKEN_OF_OWNER_BY_INDEX(owner, 0).lower()): _words_hex(_uint_word(99)),
...
def test_position_reader_returns_empty_for_invalid_wallet():
    reader = ArbitrumUniswapV3PositionReader(rpc_url="https://rpc.local", request_fn=None)
...
def test_position_reader_sets_fee_zero_when_price_feed_is_unavailable():
    ...
    slot0 = _words_hex(_uint_word(0), _int_word(1), _uint_word(0), _uint_word(0), _uint_word(0), _uint_word(0), _uint_word(1))
    mapping = {
        (pm, ArbitrumUniswapV3PositionReader._BALANCE_OF(owner).lower()): _words_hex(_uint_word(1)),
        (pm, ArbitrumUniswapV3PositionReader._TOKEN_OF_OWNER_BY_INDEX(owner, 0).lower()): _words_hex(_uint_word(7)),
